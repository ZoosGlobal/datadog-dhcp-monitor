...
#  Namespaces: dhcp.server.* | dhcp.scope.*
# ------------------------------------------------------------------------------
#  Author    : Shivam Anand
...
$null = $udpClient.Connect($dogstatsdHost, $dogstatsdPort)  # ← suppress output
...
try {
    ...
    Send-Metric "dhcp.server.total_addresses"   $v4Stats.TotalAddresses                              $v4Tags
    Send-Metric "dhcp.server.addresses_in_use"  $v4Stats.AddressesInUse                              $v4Tags
    Send-Metric "dhcp.server.addresses_free"    ($v4Stats.TotalAddresses - $v4Stats.AddressesInUse)  $v4Tags
    Send-Metric "dhcp.server.pending_offers"    $v4Stats.PendingOffers                               $v4Tags
    Send-Metric "dhcp.server.percent_in_use"    $v4Stats.PercentageInUse                             $v4Tags
    Send-Metric "dhcp.server.percent_available" $v4Stats.PercentageAvailable                         $v4Tags
    Send-Metric "dhcp.server.total_scopes"      $v4Stats.TotalScopes                                 $v4Tags

    # DHCP message/traffic counters
    # High naks/declines = IP conflicts | High discovers + low acks = scope exhaustion
    Send-Metric "dhcp.server.discovers"         $v4Stats.Discovers                                   $v4Tags
    Send-Metric "dhcp.server.offers"            $v4Stats.Offers                                      $v4Tags
    Send-Metric "dhcp.server.requests"          $v4Stats.Requests                                    $v4Tags
    Send-Metric "dhcp.server.acks"              $v4Stats.Acks                                        $v4Tags
    Send-Metric "dhcp.server.naks"              $v4Stats.Naks                                        $v4Tags
    Send-Metric "dhcp.server.declines"          $v4Stats.Declines                                    $v4Tags
    Send-Metric "dhcp.server.releases"          $v4Stats.Releases                                    $v4Tags
    Send-Metric "dhcp.server.delayed_offers"    $v4Stats.DelayedOffers                               $v4Tags
    ...
    Send-Metric "dhcp.server.uptime_seconds"    $uptime                                              $v4Tags

} catch {
    Write-Warning "[Zoos Global] Failed to collect IPv4 server stats: $_"
}
...
try {
    ...
    foreach ($scope in $v4Scopes) {
        try {
            ...
            # Address pool metrics
            Send-Metric "dhcp.scope.total_addresses"  $total                       $tags
            Send-Metric "dhcp.scope.addresses_free"   $stats.AddressesFree         $tags
            Send-Metric "dhcp.scope.addresses_in_use" $stats.AddressesInUse        $tags
            Send-Metric "dhcp.scope.percent_in_use"   $stats.PercentageInUse       $tags
            Send-Metric "dhcp.scope.pending_offers"   $stats.PendingOffers         $tags
            Send-Metric "dhcp.scope.reserved_address" $stats.ReservedAddress       $tags
            ...
            Send-Metric "dhcp.scope.free_this_server"      $stats.AddressesFreeOnThisServer     $tags
            Send-Metric "dhcp.scope.free_partner_server"   $stats.AddressesFreeOnPartnerServer  $tags
            Send-Metric "dhcp.scope.inuse_this_server"     $stats.AddressesInUseOnThisServer    $tags
            Send-Metric "dhcp.scope.inuse_partner_server"  $stats.AddressesInUseOnPartnerServer $tags

        } catch {
            Write-Warning "[Zoos Global] Failed to collect IPv4 scope stats for $($scope.ScopeId): $_"
        }
    }
} catch {
    Write-Warning "[Zoos Global] Failed to enumerate IPv4 scopes: $_"
}
...
try {
    ...
    Send-Metric "dhcp.server.addresses_in_use"  $v6Stats.AddressesInUse   $v6Tags

    # IPv6 protocol message equivalents:
    # Discover -> Solicit | Offer -> Advertise | Ack -> Reply
    Send-Metric "dhcp.server.discovers"         $v6Stats.Solicts           $v6Tags
    Send-Metric "dhcp.server.offers"            $v6Stats.Advertises        $v6Tags
    Send-Metric "dhcp.server.acks"              $v6Stats.Replies           $v6Tags
    Send-Metric "dhcp.server.releases"          $v6Stats.Releases          $v6Tags

    # Uptime in seconds
    $v6Uptime = [int](((Get-Date) - $v6Stats.ServerStartTime).TotalSeconds)
    Send-Metric "dhcp.server.uptime_seconds"    $v6Uptime                  $v6Tags

} catch {
    Write-Warning "[Zoos Global] Failed to collect IPv6 server stats: $_"
}
...
# Tags      : scope (prefix), scopename, version
# NOTE      : total_addresses / addresses_free intentionally skipped
...
try {
    ...
    foreach ($scope in $v6Scopes) {
        try {
            ...
            Send-Metric "dhcp.scope.reserved_address" $v6Reservations $tags

        } catch {
            Write-Warning "[Zoos Global] Failed to collect IPv6 scope stats for $($scope.Prefix): $_"
        }
    }
} catch {
    Write-Warning "[Zoos Global] Failed to enumerate IPv6 scopes: $_"
}

# ==============================================================================
$udpClient.Close()
Write-Host "`nDone! Check Datadog Metrics Explorer for dhcp.*"
# ==============================================================================
